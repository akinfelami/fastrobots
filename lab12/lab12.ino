...
void handle_command() {
  ...
  case SET_SETPOINT: {
    ...
    record_dmp_data();
    pid_target = yaw_gy + new_setpoint;
  ...
  case DRIVE: {
    // drive in a straight line for given time
    int time_to_drive;
    success = robot_cmd.get_next_value(time_to_drive);
    if (!success) {
      return;
    }
    drive_in_a_straight_line(1, 200, 1.25);
    delay(time_to_drive);
    stop();
    break;
  }
...
void localize() {
  ...
  for (int step = 0; step < 18; step++) {
    pid_target =
        start_yaw + (step * 20); // Increment target by 20 deg each step
  ...
  // Ensure we stop and reset all control variables
  stop();
  is_pid_running = false;
...
void loop() {
  ...
  while (central.connected()) {
    ...
    // if the pid is running, it is doing a turn
    if (is_pid_running) {
      record_dmp_data();
      int pwm = pid_control(yaw_gy);
      ...
      // if it is within 3 degrees, stop
      if (abs(yaw_gy - pid_target) < 3.0) {
        stop();
        is_pid_running = false;
      }
    }
